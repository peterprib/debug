...
module.exports = function (RED) {
  function loggerNode (n) {
    ...
    this.logger=new require("logger")("logger Node").setNodeStatus(this);
    node.on('input', function (msg) {
    	logger.send({label:"input"});
   	    if(this.logger.active) {
   	    	this.logger.send({label:"input",message:msg});
   	    	if(this.sendOutputLog)
   	    		node.send([null,msg]);
   	    }
    	node.send(msg);
    })
    node.on('close', function (removed, done) {
      node.connection.close(false, () => {
   	    if(logger.active) logger.send({label:"close"});
      })
      done()
    })
  }
